...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    scores = torch.matmul(query, key.transpose(-2, -1)) / (ahs ** 0.5)
    ...
    context = context.view(bs, seqlen, -1)
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    residual = dropped_output + input
    
    rslt = ln_layer(residual)
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    # add-norm layer
    attention_add_norm = self.add_norm(hidden_states, 
                                       attention_output, 
    ...
    interm_output = self.interm_af(self.interm_dense(attention_add_norm))

    # another add-norm layer
    
    rslt = self.add_norm(attention_add_norm, 
                        interm_output, 
                        self.out_dense, 
                        self.out_dropout, 
                        self.out_layer_norm)
